...
class AccountHistRepository:
    ...
    def search_by_dttm_and_job_div(self, session, customer_id, account_id, from_dttm, to_dttm, proc_div):
        ...
        if from_dttm is not None:
            account_hist = account_hist.filter(AccountHist.proc_dttm >= from_dttm)
        if to_dttm is not None:
            account_hist = account_hist.filter(AccountHist.proc_dttm <= to_dttm)
        if proc_div is not None:
            account_hist = account_hist.filter(AccountHist.proc_div == proc_div)
